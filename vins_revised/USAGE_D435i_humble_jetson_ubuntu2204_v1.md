...
#### 2.3 分别启动 RealSense 和 VINS（调试更方便）
...
**终端 B — 启动 VINS（自动带 `/vins_estimator/` 前缀）：**

```bash
ros2 launch vins vins_estimator.launch.py \
  config_path:=/home/lingzhilab/vins/src/VINS-Fusion-ROS2/config/realsense_d435i_vio/realsense_stereo_imu_config.yaml
```

> `vins_estimator.launch.py` 已内置所有话题 remappings，loop_fusion 可以直接订阅 `/vins_estimator/...` 话题，无需额外配置。
>
> **注意**：如果提示 `file 'vins_estimator.launch.py' was not found in the share directory`，说明 launch 文件尚未编译到 `install` 目录。需要重新编译 vins 包：
> ```bash
> cd /home/lingzhilab/vins
> source /opt/ros/humble/setup.bash
> colcon build --symlink-install --parallel-workers 1 --packages-select vins
> source install/setup.bash
> ```
> > `--symlink-install` 模式下，以后修改 launch 文件内容不需要重新编译；但如果是新建的 launch 文件，第一次必须编译才能被 `ros2 launch <pkg>` 找到。也可以临时用绝对路径直接启动：
> > ```bash
> > ros2 launch /home/lingzhilab/vins/src/VINS-Fusion-ROS2/vins/launch/vins_estimator.launch.py ...
> > ```
